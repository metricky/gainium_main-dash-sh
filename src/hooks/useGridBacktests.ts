...
import { useLocalGridBacktests } from './useLocalGridBacktests';

export interface BacktestsFilter {
  paperContext?: boolean;
}

export interface UseGridBacktestsResult {
  data: ReturnResult<GRIDBacktestingResultHistory[]> | null;
  backtests: GRIDBacktestingResultHistory[];
  total: number;
...
export function useGridBacktests(
  _filter?: BacktestsFilter
): UseGridBacktestsResult {
  const local = useLocalGridBacktests();

  // Get the query and variables from botQueries with proper input parameters
  const { query, variables } = botQueries.getGridBacktests({
    page: 0,
    pageSize: 50,
  ...
  const queryResult = useGraphQL<GRIDBacktestingResultHistory[]>(
    'getGridBacktests',
    {
      query,
      variables,
  ...
  if (queryResult.error) {
    logger.error('[useGridBacktests] Query error:', queryResult.error.message);
  }

  // Handle the response structure properly
  let backtestResponse: GRIDBacktestingResultHistory[] | null = null;
  let responseTotal = 0;
  ...
  const mergedBacktests = useMemo(() => {
    const byId = new Map<string, GRIDBacktestingResultHistory>();
    const withoutId: GRIDBacktestingResultHistory[] = [];
  ...
  if (transformedBacktests.length > 0) {
    logger.debug('[useGridBacktests] First backtest:', transformedBacktests[0]);
  }
