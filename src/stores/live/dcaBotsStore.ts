...
interface DcaBotsStoreState {
  // Bots by bot ID
  bots: Record<string, DCABot>;
  ...
  // Actions
  addBot: (bot: DCABot) => void;
  updateBots: (bots: DCABot[]) => void;
  updateBot: (bot: DCABot) => void;
  ...
  // Selectors
  getBot: (botId: string) => DCABot | null;
  getAllBots: () => DCABot[];
  getBotsByStatus: (statuses: string[]) => DCABot[];
  isLoading: () => boolean;
...
export const useDcaBotsStore = create<DcaBotsStoreState>()(
  devtools(
    persist(
      (set, get) => ({
        bots: {},
        ...
        updateBots: (bots: DCABot[]) => {
          ...
          const botsRecord: Record<string, DCABot> = {};
          bots.forEach((bot) => {
        ...
        updateBot: (bot: DCABot) => {
          const existingBot = get().bots[bot._id];

          // Only update if bot already exists in store
          if (!existingBot) {
            logger.debug(
              `[DcaBotsStore] Skipping update for non-existent bot ${bot._id}`
            );
            return;
          }
          ...
          updates.forEach(({ botId, data, paperContext }) => {
            ...
            if (data.updated && existingBot.updated) {
              ...
              if (wsTime < existingTime) {
                ...
                logger.debug(
                  `[DcaBotsStore] Skipping stale WebSocket update for bot ${botId}`,
                  { wsTime, existingTime }
                );
                return;
              }
            ...
            if (
              ...
              logger.debug(
                `[DcaBotsStore] Skipping cross-context WebSocket update for bot ${botId}`,
                {
                  paperContext,
      ...
      }),
      {
        ...
        merge: (persistedState, currentState) => ({
          ...currentState,
          ...(persistedState as Partial<DcaBotsStoreState>),
        ...
        onRehydrateStorage: () => (_state, error) => {
          if (error) {
            logger.error('[DcaBotsStore] Rehydration error:', error);
          }
          useDcaBotsStore.getState().setHasHydrated(true);
        },
      }
    ),
    {
