...
export const verifyNumber = (_str: string) => {
  let str = `${_str}`;
  if (str.indexOf('e') !== -1) {
    str = math.convertFromExponential(_str, 20);
  }
  return /^-?\d*\.?\d*$/g.test(str);
};
export const trimNumber = (_str: string, int = false, abs = false) => {
  const str = `${_str}`;
  if (str.indexOf('e') !== -1) {
    return math.convertFromExponential(str, 20);
  }
  const numb = int ? parseInt(str) : parseFloat(str);
  if (isNaN(numb)) {
    return '';
  }
  return abs ? `${Math.abs(numb)}` : `${numb}`;
