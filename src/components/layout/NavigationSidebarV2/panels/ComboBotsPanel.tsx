...
const ComboBotsPanel: React.FC<ComboBotsPanelProps> = ({
  ...
  const handleBotClick = (bot: any) => {
    ...
    const base = getBotTypeRoute(type || 'combo');
    ...
    <RightPanel
      ...
      headerActions={
        <div className="flex items-center gap-2">
          <button
            title="Create Combo bot"
            aria-label="Create Combo bot"
            onClick={() => {
              navigate('/combo/new');
              onNavigate?.();
              if (!navigationSecondaryPinned) onClose();
          ...
          >
            <span className="sr-only">Create Combo bot</span>
            <svg
              className="h-4 w-4"
              viewBox="0 0 24 24"
              fill="none"
