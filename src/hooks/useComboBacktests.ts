...
export interface UseComboBacktestsResult {
...
export function useComboBacktests(
  _filter?: BacktestsFilter
): UseComboBacktestsResult {
  ...
  if (queryResult.error) {
    logger.error('[useComboBacktests] Query error:', queryResult.error.message);
  }
  ...
  logger.debug('[useComboBacktests] Summary:', {
  ...
  if (transformedBacktests.length > 0) {
    logger.debug(
      '[useComboBacktests] First backtest:',
      transformedBacktests[0]
    );
  }
